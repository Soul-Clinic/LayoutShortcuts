...
#define kMaxVelocityForQuiet	50.0
#define kQuietTimeBeforeMove	0.2
...
- (IBAction)handlePanGesture:(UIPanGestureRecognizer *)pan
{
	...
	if (_panningButton && _panningButton != btn) {		//Multiple touches
		return;
	}
	...
	switch (pan.state) {
		case UIGestureRecognizerStateBegan:
		{
			...
			layer.zPosition = 100;
			_panningButton = btn;
			[layer addAnimation:[self scaleAnimationFactor:kPanScaleFactor duration:0.2 * kStandDuration] forKey:kScaleAnimationKey];
		...
		case UIGestureRecognizerStateChanged:
		{
			...
			if (_destination != _current && linearVelocity < kMaxVelocityForQuiet) {
				if (trigger == NO) {
...
- (IBAction)handleLongPressGesture:(UILongPressGestureRecognizer *)longPress
{
	...
	switch (longPress.state) {
		case UIGestureRecognizerStateBegan:
			...
			[layer addAnimation:[self scaleAnimationFactor:kPanningAlpha duration:0.2 * kStandDuration] forKey:kScaleAnimationKey];
			_zPosition = layer.zPosition;
			layer.zPosition = 100;
		...
		case UIGestureRecognizerStateEnded:
		{
			...
			[longPress.view.layer removeAnimationForKey:kScaleAnimationKey];
